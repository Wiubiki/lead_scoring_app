...
from utils.supabase_client import save_scoring_run
...
def render():
    ...
    # --------------------------
    # Scored Leads Table Panel (Basic)
    # --------------------------
    with st.expander("Scored Leads", expanded=False):
    ...
    if st.button("💾 Save scoring run to Supabase", type="primary"):
        ...
        if not period_start or not period_end:
            st.error("Scoring period is missing. Please re-run scoring.")
        else:
            try:
                run_row = save_scoring_run(scored_df, period_start, period_end)
                st.success(
                    f"Saved scoring run to Supabase: "
                    f"{run_row['period_start']} → {run_row['period_end']} "
                    f"({run_row['lead_count']} leads)."
                )
            except Exception as e:
                st.error(f"Failed to save scoring run: {e}")
